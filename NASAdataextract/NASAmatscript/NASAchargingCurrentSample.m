...
for i = 1:length(big_battery_folders)
    ...
    % 初始化存储合并数据的表
    combined_charge_data = table([], [], [], [], 'VariableNames', {'Time', 'Current', 'Cycle Life', 'Battery Name'});
    combined_discharge_data = table([], [], [], [], 'VariableNames', {'Time', 'Current', 'Cycle Life', 'Battery Name'});
    ...
    for k = 1:length(mat_files)
        ...
        % 检查并提取 charge 和 discharge 的 measured_current
        if isfield(main_data, 'cycle')
            ...
            for cycle_idx = charge_cycles
                ...
                current_data = cycle.data.Current_measured(:);
                time_data = cycle.data.Time(:);
                cycle_life_data = repmat(cycle_idx, length(time_data), 1);
                battery_name_data = repmat({mat_files(k).name}, length(time_data), 1);
                cycle_data = table(time_data, current_data, cycle_life_data, battery_name_data, 'VariableNames', {'Time', 'Current', 'Cycle Life', 'Battery Name'});
    ...
    outputChargeFolder = '/Users/jonathanzha/Desktop/Battery-dataset-preprocessing-code-library/NASA/BatteriesChargeCurrent';
    outputDischargeFolder = '/Users/jonathanzha/Desktop/Battery-dataset-preprocessing-code-library/NASA/BatteriesDischargeCurrent';

    % 保存合并后的数据到一个新的CSV文件
    % 保存合并后的数据到一个新的CSV文件
    charge_output_csv = fullfile(outputChargeFolder, sprintf('batch%d.csv', i));
    discharge_output_csv = fullfile(outputDischargeFolder, sprintf('batch%d.csv', i));
