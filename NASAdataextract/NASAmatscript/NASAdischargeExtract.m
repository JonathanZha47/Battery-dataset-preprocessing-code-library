...
for i = 1:length(big_battery_folders)
    ...
    for k = 1:length(mat_files)
        ...
        % 初始化存储合并数据的表
        combined_data = table([], [], [], [], [], [], [], [],'VariableNames', {'Time', 'Voltage_Measured', 'Current_Measured', 'Voltage_Act','Current_Act', 'Temperature','Capacity', 'Cycle Life'});
        ...
        if isfield(main_data, 'cycle')
            ...
            for cycle_idx = discharge_cycles
                ...
                % 检查容量数据是否存在
                if isempty(cycle.data.Capacity)
                    warning('Skipping cycle %d in file %s due to missing capacity data.', cycle_idx, mat_files(k).name);
                    continue;
                end
                voltage_measured_data = cycle.data.Voltage_measured(:);
                current_measured_data = cycle.data.Current_measured(:);
                voltage_actual_data = cycle.data.Voltage_load(:);
                current_actual_data = cycle.data.Current_load(:);
                time_data = cycle.data.Time(:);
                capacity_data = repmat(cycle.data.Capacity, length(time_data), 1);
                cycle_life_data = repmat(cycle_idx, length(time_data), 1);
                temperature_data = cycle.data.Temperature_measured(:);
               
                
                % 确保所有列的长度一致
                if length(voltage_measured_data) == length(current_measured_data) && ...
                   length(voltage_measured_data) == length(voltage_actual_data) && ...
                   length(voltage_measured_data) == length(current_actual_data) && ...
                   length(voltage_measured_data) == length(time_data) && ...
                   length(voltage_measured_data) == length(temperature_data) && ...
                   length(voltage_measured_data) == length(capacity_data) && ...
                   length(voltage_measured_data) == length(cycle_life_data)
                   
                    cycle_data = table(time_data, voltage_measured_data, current_measured_data, voltage_actual_data, current_actual_data, temperature_data, capacity_data, cycle_life_data, ...
                                       'VariableNames', {'Time', 'Voltage_Measured', 'Current_Measured', 'Voltage_Act', 'Current_Act', 'Temperature', 'Capacity', 'Cycle Life'});
                    combined_data = [combined_data; cycle_data];
                else
                    warning('Data length mismatch in file: %s, cycle: %d', mat_files(k).name, cycle_idx);
                end
            end
        end
        
        % 保存合并后的数据到一个新的CSV文件
        outputFolder = '/Users/jonathanzha/Desktop/Battery-dataset-preprocessing-code-library/NASA/DischargeExtract';
        if ~exist(outputFolder, 'dir')
            mkdir(outputFolder);
        end
        output_csv = fullfile(outputFolder, sprintf('%s_DischargeExtract.csv', mat_files(k).name));
